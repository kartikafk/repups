...
export default function TrainerChat({ initialTrainerId }) {
  ...
  useEffect(() => {
    ...
    const fetchTrainerProfile = async () => {
      try {
        const res = await fetch(`http://localhost:5001/api/trainers/${initialTrainerId}`);
  ...
  useEffect(() => {
    const fetchConversations = async () => {
      if (!initialTrainerId) return;
      try {
        const res = await fetch(`http://localhost:5001/api/messages/conversations/${initialTrainerId}`, {
  ...
  useEffect(() => {
    ...
    const fetchThreadAndSlots = async () => {
      try {
        // Fetch thread
        const threadRes = await fetch(`http://localhost:5001/api/messages/thread?trainerId=${activeTrainer}&clientId=${clientId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        ...
        const slotsRes = await fetch(`http://localhost:5001/api/trainers/${activeTrainer}/slots`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        const slotsData = await slotsRes.json();
        if (slotsData.success) {
          setTrainerSlots(slotsData.slots || []);
        }
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", height: "100vh" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", borderBottom: `1px solid ${C.border}` }}>
            ...
            <button onClick={() => setShowSlotPicker(true)} style={{ background: "transparent", border: `1px solid ${C.lime}`, color: C.lime, borderRadius: 8, padding: "7px 12px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
              🎥 Schedule call
            </button>
          </div>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
                {trainerSlots.length === 0 ? (
                  ...
                  trainerSlots.map(slot => (
                    <button key={slot._id || slot} onClick={() => handleConfirmBooking(slot._id || slot)} style={{
