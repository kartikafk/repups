...
export function usePoseTracker({ exercise, voiceOn }) {
  ...
  const holdStateRef = useRef('waiting');      
  const holdStartRef = useRef(0);
  const holdSessionsRef = useRef([]);          
  const holdIssueFlagsRef = useRef({});        
  ...
  const balanceRef = useRef({ left: 50, right: 50 });
  ...
  function detectCameraView(lm) {
    const shoulderWidth = Math.abs(lm[LM.L_SHOULDER].x - lm[LM.R_SHOULDER].x);
    const hipWidth = Math.abs(lm[LM.L_HIP].x - lm[LM.R_HIP].x);
    if (shoulderWidth < 0.10 && hipWidth < 0.08) return "side";
    return "front";
  }
  ...
  function completeRep(angles) {
    ...
    if (c.compoundJoints) {
      c.compoundJoints.forEach(joint => {
          ...
          if (minVal !== undefined) {
              if (minVal < joint.min || minVal > joint.max) {
                  score -= 15;
                  flags.push(joint.flag);
                  noteIssue(joint.flag, issueDescription(joint.flag));
              }
          }
      });
    }

    // Reset compound tracker for the next rep
  ...
  function processAngle(angle, angles, lm, now){
    ...
    if (movementQualityRef.current.length > 120) movementQualityRef.current.shift();
    if (movementQualityRef.current.length >= 30) {
        const recent = movementQualityRef.current.slice(-30);
        const averageQuality = recent.reduce((a, b) => a + b, 0) / recent.length;
        fatigueIndexRef.current = Math.max(0, Math.min(100, 100 - averageQuality));
    }
    ...
    if (state === 'top' && angle < c.topAngle - 8 && velocity < -PAUSE_VELOCITY_THRESHOLD) {
      ...
      if (c.compoundJoints) {
          c.compoundJoints.forEach(joint => {
              ...
              if (currentVal !== undefined) {
                  minCompoundAnglesRef.current[joint.name] = Math.min(
                      minCompoundAnglesRef.current[joint.name] || 999, 
                      currentVal
                  );
              }
          });
      ...
      if (state === 'descending' && Math.abs(velocity) < PAUSE_VELOCITY_THRESHOLD && angle < c.topAngle - 20) {
        phaseDurRef.current.ecc = now - phaseStartRef.current.ecc;
        phaseStartRef.current.pause = now;
        state = 'paused';
      } else if (state === 'paused' && velocity > PAUSE_VELOCITY_THRESHOLD) {
  ...
  function processSideAngle(angles, lm, now) {
    ...
    switch (exercise) {
        ...
        case "lunge":
            ...
            break;
        case "deadlift":
            primaryAngle = leftVisible ? angles.leftHipAngle : angles.rightHipAngle;
            break;
        case "shoulderPress":
  ...
  function drawSkeleton(lm) {
    ...
    const headRadius=Math.max(
        18,
        Math.hypot(pts.L_EAR.x-pts.R_EAR.x, pts.L_EAR.y-pts.R_EAR.y)*.7
    );

    ctx.beginPath();
    ctx.strokeStyle="#ffffff";
    ctx.lineWidth=4;
    ctx.arc(V.head.x, V.head.y, headRadius, 0, Math.PI*2);
    ctx.stroke();
    ...
    bone(pts.L_SHOULDER, pts.R_SHOULDER, 5);
    bone(pts.L_SHOULDER, V.chest, 3);
    bone(pts.R_SHOULDER, V.chest, 3);
    bone(pts.L_SHOULDER, pts.L_ELBOW);
    bone(pts.L_ELBOW, pts.L_WRIST);
    bone(pts.R_SHOULDER, pts.R_ELBOW);
    bone(pts.R_ELBOW, pts.R_WRIST);
    bone(pts.L_HIP, pts.R_HIP, 5);
    bone(V.lowerSpine, pts.L_HIP, 3);
    bone(V.lowerSpine, pts.R_HIP, 3);
    bone(pts.L_HIP, pts.L_KNEE);
    bone(pts.L_KNEE, pts.L_ANKLE);
    bone(pts.R_HIP, pts.R_KNEE);
    bone(pts.R_KNEE, pts.R_ANKLE);
    bone(V.head, V.neck, 4);
    bone(V.neck, V.chest, 3);
    ...
    if (motionTrailRef.current.length > 2) {
        drawTrail(ctx, motionTrailRef.current.map(p));
    }
    ...
    drawAngleText(ctx, pts.L_HIP.x, pts.L_HIP.y, angles.hipAngle);
    ...
    rows.forEach((r, i) => {
        ...
        ctx.fillStyle = value > 85 ? "#66FF66" : value > 70 ? "#FFD966" : "#FF6666";
    ...
    ctx.fillText(`Sym ${angles.kneeAngleAsym.toFixed(1)}°`, 20, 135);
    ctx.fillStyle = "#66FF66";
    ctx.fillText(`Neck ${angles.neckAngle.toFixed(1)}°`, 20, 155);
  }
  ...
  const start = useCallback(async (facingMode = "user", videoElement = null) => {
    ...
    const startPromise = (async () => {
      ...
      setCue({ text: 'Get in frame to begin', kind: 'go' });
      ...
      try {
        ...
        if (!video) throw new Error('Video element disappeared before attaching camera.');
        ...
        try {
          ...
          if ('srcObject' in video) {
            ...
            try { video.src = window.URL.createObjectURL(stream); } catch (e) { video.src = ''; }
          }
          video.muted = true;
          video.autoplay = true;
        ...
        if (!video.srcObject) throw new Error('Camera stream was not attached to the video element.');

        await new Promise((resolve, reject) => {
          const finish = async () => {
            try { await video.play().catch(() => {}); resolve(); } catch (err) { reject(err); }
          };
          const onMetadata = () => {
          ...
          if (video.readyState >= 2 || video.videoWidth > 0 || video.videoHeight > 0) {
            ...
            window.setTimeout(() => {
              if (video.readyState >= 2 || video.videoWidth > 0 || video.videoHeight > 0) onMetadata();
              else finish();
            }, 3000);
          }
        });
  ...
  const buildReport = useCallback(() => {
    ...
    return {
      ...
      repCount: n,
      avgScore,
      avgTempo: { ecc: avgEcc, pause: avgPause, con: avgCon, replay: replayURLRef.current },
      ...
      movementQuality: movementQualityRef.current.length
          ? movementQualityRef.current.reduce((a, b) => a + b, 0) / movementQualityRef.current.length : 0,
      stability: stabilityHistoryRef.current.length
          ? stabilityHistoryRef.current.reduce((a, b) => a + b, 0) / stabilityHistoryRef.current.length : 0,
      symmetry: symmetryHistoryRef.current.length
          ? symmetryHistoryRef.current.reduce((a, b) => a + b, 0) / symmetryHistoryRef.current.length : 0
    };
  ...
  const tracker = useMemo(
    () => ({
      videoRef, canvasRef, start, stop, buildReport, clearReplay,
      repCount, rom, tempo, phase, cue, hudStatus,
      isHold: isHoldExercise(), holdTime, bestHold, holdCount,
      jointScores: jointScoresRef.current, balance: balanceRef.current, fatigue: fatigueIndexRef.current,
      movementQuality: movementQualityRef.current.length ? movementQualityRef.current.reduce((a,b)=>a+b,0) / movementQualityRef.current.length : 0,
      stability: stabilityHistoryRef.current.length ? stabilityHistoryRef.current.reduce((a,b)=>a+b,0) / stabilityHistoryRef.current.length : 0,
      symmetry: symmetryHistoryRef.current.length ? symmetryHistoryRef.current.reduce((a,b)=>a+b,0) / symmetryHistoryRef.current.length : 0,
      lastReplay: replayURLRef.current,
    }),
    [start, stop, buildReport, clearReplay, repCount, rom, tempo, phase, cue, hudStatus, isHoldExercise, holdTime, bestHold, holdCount]
  );
  return tracker;
}